...
def hangman():
    ...
    used_letters = set()  # Keeping track of what users have guessed
    lives = 6

    # Getting user input using loop
    while len(word_letters) > 0 and lives > 0:
        # Print letters that are already used
        print("\nLives left: ", lives, "You have used these letters: ", " ".join(used_letters))

        # Displaying the chars in the word where user has guessed it correctly
        word_list = [letter if letter in used_letters else "-" for letter in word]
        print("Current word: ", " ".join(word_list))

        usr_ltr = input("Enter your guess: ").upper()

        if usr_ltr in alphabet - used_letters:
            used_letters.add(usr_ltr)

            if usr_ltr in word_letters:
                word_letters.remove(usr_ltr)

            else:
                lives = lives - 1
                print("\nLetter is not in the word")

        elif usr_ltr in used_letters:
            print("\nYou have already used that letter for your word")

        else:
            print("\nInvalid input")

    if lives == 0:
        print(" \nGame Over!!, you did not guess the word: ", word)
    else:
        print("\nYou have guessed the word correctly: ", word)
